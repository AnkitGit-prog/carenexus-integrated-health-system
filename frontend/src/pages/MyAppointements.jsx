...
const MyAppointements = () => {
  ...
  const [appointments, setAppointments,getDoctorsData] = useState([])
...
// ye delete kare ka logic hai fontend ka
      ...
      <div>
        {appointments.map((item, index) => (
          <div className='grid grid-cols-[1fr_2fr] gap-4  sm:flex sm:gap-6 py-2 border-b' key={item._id || index}>
           ...
           { !item.cancelled &&  <button onClick ={()=>cancleAppointment(item._id)}className='text-sm text-stone-500 text-center sm:min-w-48 py-2 border  hover:bg-red-600 hover:text-white transition-all duration-300'>Cancel appointment</button>}
           
           { item.cancelled===true && <button className ='sm:min-w-48  py-2 border border-red-400 rounded text-red-500 '> Appointement cancle</button>}
            </div>
          </div>
        ))}
      </div>
